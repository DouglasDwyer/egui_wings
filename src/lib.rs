...
impl dyn Egui {
    pub fn context(&self) -> EguiHandle {
        ...
        let SerializedViewport::Owned(state) = self.get_state() else { panic!("Received context.") };
...
pub enum SerializedViewport {
    FromContext(Context),
    Owned(SerializedViewportOwned)
}

impl Serialize for SerializedViewport {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        match self {
            SerializedViewport::FromContext(ctx) => {
                let mut sequence = serializer.serialize_struct("SerializedViewportOwned", 10)?;
                
                unsafe { ctx.memory(|x| sequence.serialize_field("memory", transmute::<_, &SerializedMemory>(x)))? };

                sequence.serialize_field("style", &ctx.style());
                ctx.viewport(|x| {
            ...
            SerializedViewport::Owned(x) => <SerializedViewportOwned as Serialize>::serialize(x, serializer),
        }
    }
}
...
impl<'de> Deserialize<'de> for SerializedViewport {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        Ok(Self::Owned(<SerializedViewportOwned as Deserialize>::deserialize(deserializer)?))
    }
}
...
impl<'a> Drop for EguiHandle<'a> {
    fn drop(&mut self) {
        self.ctx.set_state(SerializedViewport::FromContext(self.clone()));
    }
}
